...
##########################################################
            # temp_list = []
            # for j in range(itp_data.shape[1]):
            #     x = itp_data[:, j]
            #     xf = torch.fft.rfft(x)
            #     pass_f = torch.abs((torch.fft.rfftfreq(x.shape[0]))) > 0.3
            #     rx = torch.fft.irfft(xf * pass_f, n=x.shape[0])

            #     temp_list.append(rx)
            # dataf = torch.stack(temp_list, dim=1)
            # s["coeffs"] = dataf.numpy()
##########################################################
            # temp_list = []
            # for j in range(itp_data.shape[1]):
            #     x_j = itp_data[:, j]
            ...
            #     freq = abs(xf)
            #     __, toplist = torch.topk(freq, 10)

            #     for x in range(xf.shape[0]):
            #             if x not in toplist:
            #                 xf[x] = 0
                
            #     rx = torch.fft.irfft(xf, n=x_j.shape[0])
            #     temp_list.append(rx)
            # dataf = torch.stack(temp_list, dim=1)
            # s["coeffs"] = dataf.numpy()
##########################################################
...
if __name__ == '__main__':
    ...
    train_loader, valid_loader, test_loader, scaler, mean_scaler = get_dataloader(
        batch_size=16, device="cuda:1", val_len=0.1,
        is_interpolate=True, num_workers=40,
    ...
    for month_idx in range(len(test_dataset.gt_mask)):
        gt_mask = test_dataset.gt_mask[month_idx]         # ground truth mask
        ob_mask = test_dataset.observed_mask[month_idx]   # actual observed mask

        # Only consider positions that are:
        # - gt_mask == 0  → marked missing
        # - ob_mask == 1 → data originally exists (can be evaluated)
        effective_missing = (gt_mask == 0) & (ob_mask == 1)
        effective_total = ob_mask == 1  # total valid positions

        effective_missing_count += np.sum(effective_missing)
        effective_total_count += np.sum(effective_total)

    missing_rate = effective_missing_count / effective_total_count

    print(f"\n[Effective Missing Rate in Test Set]")
    print(f"Total evaluable positions (observed == 1): {effective_total_count}")
    print(f"Artificial missing positions (gt == 0 and observed == 1): {effective_missing_count}")
    print(f"Effective missing rate: {missing_rate:.4%}")
