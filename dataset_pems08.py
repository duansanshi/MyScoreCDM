...
def get_mean_std():
    df = pd.HDFStore("./data/PEMS08/pems08.h5").get('df')
    data_len = len(df)
    train_data = df[:int(data_len*0.7)].values
    mean = np.mean(train_data, 0)
    std = np.std(train_data, 0)
    with open('./data/PEMS08/pems08_meanstd.pk', 'wb') as f:
...
class Pems08_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        df = pd.read_hdf("./data/PEMS08/pems08.h5")
        ob_mask = (df.values != 0.).astype('uint8')
        SEED = 9101112
        self.rng = np.random.default_rng(SEED)
        if missing_pattern == 'block':
            eval_mask = sample_mask(shape=(17856, 170), p=0.0015, p_noise=0.05, min_seq=12, max_seq=12 * 4, rng=self.rng)
            if missing_ratio is not None:
                eval_mask = sample_mask(shape=(117856, 170), p=missing_ratio, p_noise=0.05, min_seq=12, max_seq=12 * 4,
                                        rng=self.rng)
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(17856, 170), p=0., p_noise=0.30, max_seq=12, min_seq=12 * 4, rng=self.rng)
...
if __name__ =='__main__':
    ...
    # 打开 HDF5 文件并获取数据集
    with pd.HDFStore("./data/PEMS08/pems08.h5") as store:
    ...
    #eval_mask = sample_mask(shape=(17856, 170), p=0.0015, p_noise=0.05, min_seq=12, max_seq=12 * 4, rng=rng)
    eval_mask = sample_mask(shape=(17856, 170), p=0., p_noise=0.25, max_seq=12, min_seq=12 * 4, rng=rng)
    gt_mask = (1-(eval_mask | (1-ob_mask))).astype('uint8')
    # if missing_pattern == 'block':
