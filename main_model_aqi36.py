...
from diff_models_aqi36 import Guide_diff
#from diff_models_pems08 import Guide_diff
#from diff_models_pems04 import Guide_diff
...
class Score(nn.Module):
    ...
    def DDIMimpute(self, observed_data, cond_mask, side_info, n_samples,itp_info,eta=0):
            ...
            for s in range(0,len(DDIM_seq1)):
                ...
                if s == -1:
                    ...
                    if not self.use_guide:
                        ...
                        diff_input = ((1 - cond_mask) * current_sample).unsqueeze(1)  # (B,1,K,L)
                    #predicted = self.diffmodel(diff_input,torch.tensor(t).to(self.device), side_info, itp_info,cond_mask)  
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            samples = self.DDIMimpute(observed_data, cond_mask, side_info, n_samples, itp_info)
...
class Score_Pems08(Score):
    def __init__(self, config, device, target_dim=170, seq_len=24):
        super(Score_Pems08, self).__init__(target_dim, seq_len, config, device)
        self.config = config
...
class Score_Pems04(Score):
    def __init__(self, config, device, target_dim=307, seq_len=24):
        super(Score_Pems04, self).__init__(target_dim, seq_len, config, device)
        self.config = config
