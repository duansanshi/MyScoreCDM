...
class PemsBAY_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.0, test_len=0.2, missing_pattern='block',
                 is_interpolate=False, target_strategy='random'):
        self.eval_length = eval_length
        self.is_interpolate = is_interpolate
        self.target_strategy = target_strategy
        self.mode = mode
        path = "../Score-CDM/data/pm25/pemsbay_meanstd.pk"
        ...
        if missing_pattern == 'block':
            eval_mask = sample_mask(shape=(52116, 325), p=0.05, p_noise=0.05, min_seq=12, max_seq=12 * 4, rng=None)
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(52116, 325), p=0., p_noise=0.25, max_seq=12, min_seq=12 * 4, rng=None)
