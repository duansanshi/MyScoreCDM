...
class Rearrange(BaseTransform):
    """Rearrange all keys in :class:`~tsl1.data.Data` according to the provided
    pattern using `einops.rearrange <https://einops.rocks/api/rearrange/>`_.

    If the objects is of type :class:`~tsl1.data.StaticBatch`, then the batch
...
class NodeThenTime(Rearrange):
    """Rearrange all keys in :class:`~tsl1.data.Data` such that the node
