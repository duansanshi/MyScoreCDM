...
class ConditionalBlock(Module):
    ...
    def forward(self, x, u=None):
        ...
        out = self.out_inputs_affinity(out) + self.out_cond_affinity(
            conditions)
...
class ConditionalTCNBlock(nn.Module):
    ...
    :class:`tsl1.nn.blocks.encoders.ConditionalBlock` but using temporal
    ...
    def __init__(self,
        ...
        if gated:
            ...
            self.inputs_conv = nn.Sequential(
                TemporalConv(input_channels=input_size,
                             ...
                             dilation=dilation,
                             weight_norm=weight_norm),
                get_layer_activation(activation)(), nn.Dropout(dropout))
            self.conditions_conv = nn.Sequential(
        ...
        if skip_connection:
            self.skip_conn = TemporalConv(input_size,
                                          output_size,
                                          1,
                                          channel_last=channel_last)
        else:
