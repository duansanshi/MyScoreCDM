...
import tsl
from tsl.ops.pattern import check_pattern, outer_pattern, take
...
class Scaler:
    r"""Base class for linear :class:`~tsl1.data.SpatioTemporalDataset` scalers.

    A :class:`~tsl1.data.preprocessing.Scaler` is the base class for
    ...
    def save(self, filename: str, make_dir: bool = True) -> str:
        ...
        params = self.params()
        is_torch = any(
            [isinstance(param, Tensor) for param in params.values()])
    ...
    def load(cls, filename: str) -> "Scaler":
        ...
        if ext == 'npz':
            ...
            raise RuntimeError(
                f"Filename {filename} is not in a valid format.")
        return cls(**params)
...
class StandardScaler(Scaler):
    ...
    def fit(self, x: TensArray, mask=None, keepdims=True):
        ...
        if mask is not None:
            ...
            self.bias = np.nanmean(x.astype(np.float32),
                                   axis=self.axis,
                                   keepdims=keepdims).astype(x.dtype)
            self.scale = np.nanstd(x.astype(np.float32),
                                   axis=self.axis,
                                   keepdims=keepdims).astype(x.dtype)
        else:
...
class MinMaxScaler(Scaler):
    ...
    def fit(self, x: TensArray, mask=None, keepdims=True):
        ...
        if mask is not None:
            x = np.where(mask, x, np.nan)
            x_min = np.nanmin(x.astype(np.float32),
                              axis=self.axis,
                              keepdims=keepdims).astype(x.dtype)
            x_max = np.nanmax(x.astype(np.float32),
                              axis=self.axis,
                              keepdims=keepdims).astype(x.dtype)
        else:
...
class RobustScaler(Scaler):
    ...
    def fit(self, x: TensArray, mask=None, keepdims=True):
        ...
        if not 0 <= q_min <= q_max <= 100:
            raise ValueError("Invalid quantile range: {}".format(
                self.quantile_range))
        ...
        if mask is not None:
            ...
            min_q, max_q = np.nanpercentile(x,
                                            self.quantile_range,
                                            axis=self.axis,
                                            keepdims=keepdims)
        else:
            self.bias = np.median(x, axis=self.axis, keepdims=keepdims)
            min_q, max_q = np.percentile(x,
                                         self.quantile_range,
                                         axis=self.axis,
                                         keepdims=keepdims)
...
class ScalerModule(Module):
    ...
    def extra_repr(self) -> str:
        s = [
            "bias={}".format(tuple(self.bias.shape)),
    ...
    def numpy(self):
        r"""Transform :class:`~tsl1.data.preprocessing.ScalerModule` to
        :class:`~tsl1.data.preprocessing.Scaler`."""
    ...
    def slice(self,
        ...
        :class:`~tsl1.data.preprocessing.ScalerModule`. The parameters of the
    ...
    def cat(cls,
        ...
        pattern = outer_pattern([
            scaler.pattern for scaler in scalers
            if scaler is not None and scaler.pattern is not None
        ])
