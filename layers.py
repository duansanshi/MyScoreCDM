...
class TransformerEncoderLayer_QKV(nn.Module):
    ...
    def forward(self, query, key, src, src_mask=None, src_key_padding_mask=None):
        ...
        query1  = query.permute(1, 0, 2)

        period_list, period_weight = FFT_for_Period(query1, self.k)

        I = torch.eye(24).cuda(3)
        e = torch.zeros(24).cuda(3)
        ...
        #     i = i-1
        #     e[i] = e[i] + period_weight[0,j]
        #     j = j + 1
        #     I += torch.eye(24, k=i, dtype=np.uint8)
        #     I += torch.eye(24, k=24 - i, dtype=np.uint8)
        #
        #
        # src_mask = I
        ...
        # query = contract('ll,lnc->lnc', I, query)
