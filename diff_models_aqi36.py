...
from layers import *
from transformer1_aqi36 import *
...
class NoiseProject(nn.Module):
    def __init__(self, side_dim, channels, diffusion_embedding_dim, nheads, target_dim, proj_t, order=2, include_self=True,
        ...
        self.update_gate = MessagePN2(c_in=36, c_out=36, heads=nheads, layers=1, channels=channels)
        self.spa_gate = Messagespa(c_in=36, c_out=36, heads=nheads, layers=1, channels=channels)
        self.tcn = TCN(36, 36, [36,36,36])
        #self.tcn = TemporalConvNet( 36, [36,36,36])
        adj1 = torch.randn(36,36).to(device)
